...
>
	{#each days as day}
		...
		>
			<button
				class="day--label"
				class:selected={areDatesEquivalent(day.date, selected) || areDatesEquivalent(day.date, selected2)}
				...
				class:between-selected={day.date > Math.min(...[selected, selected2]) && day.date < Math.max(...[selected, selected2])}
				type="button"
...
<style>
	...
	.day--label {
		...
		width: 100%;
		position: relative;
		border: 1 solid var(--day-border-color);
		align-items: center;
